#include <cstdio>
#include <filesystem>
#include <iostream>

#include "curl_http.hpp"
#include "api2captcha.hpp"

using namespace std;

int main (int ac, char ** av)
{
   if (ac < 2)
   {
      printf ("Usage: ./vk \"API KEY\"\n");
      return 0;
   }

   string current_path = filesystem::current_path().c_str();
   string body = current_path + "/images/temu/body.jpg";
   string part1 = current_path + "/images/temu/part1.jpg";
   string part2 = current_path + "/images/temu/part2.jpg";
   string part3 = current_path + "/images/temu/part3.jpg";

   api2captcha::curl_http_t http;
   http.set_verbose (true);

   api2captcha::client_t client;
   client.set_http_client (&http);
   client.set_api_key (av[1]);

   std::string method = "temuimage";
   const char *c_strMethod = method.c_str();

   api2captcha::temu_t cap (c_strMethod);
   cap.set_body(body);
   cap.set_part1(part1);
   cap.set_part2(part2);
   cap.set_part3(part3);

   try
   {
      client.solve (cap);
      printf ("code '%s'\n", cap.code ().c_str ());
   }
   catch (std::exception & e)
   {
      fprintf (stderr, "Failed: %s\n", e.what ());
   }

   return 0;   
}
